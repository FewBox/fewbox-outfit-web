...
const Outfit = (props: IOutfitProps): JSX.Element => {
    ...
    const [state, setState] = useState<IOutfitStates>({ tapeSize: 1, measurementType: MeasurementType.Chalk });
    const toolWidth = '12em';
    const toolHeight = '18em';
    return <Den.Components.X gap='0.6em'>
        <Den.Components.YTop width={toolWidth} height={toolHeight} gap='1em'>
        ...
        <MaskImage imageUrl="https://img.ltwebstatic.com/images3_pi/2025/01/03/7c/1735896447c6ae06db2def0b14704e23fba94b84b4.webp" maskSize={state.tapeSize * 20} isRevert={state.measurementType == MeasurementType.Eraser} />
        <Den.Components.YTop width={toolWidth} height={toolHeight} gap='1em'>
            ...
            <Den.Components.Y cross={Den.Components.YCrossType.Center}>
                ...
                <Den.Components.VRange className='tape' isValueShow backgroundColor={Den.Components.ColorType.Transparent} min={1} max={4} onChange={(e) => { setState({ ...state, tapeSize: e.target.value }) }} value={1}
                ...
                <Den.Components.VEllipse width={`${state.tapeSize * 40}px`} height={`${state.tapeSize * 40}px`} backgroundColor={Den.Components.ColorType.Placeholder} />
            </Den.Components.Y>
        </Den.Components.YTop>
    </Den.Components.X >;
};
